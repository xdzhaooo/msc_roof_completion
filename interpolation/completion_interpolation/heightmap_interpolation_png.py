...
class HeightmapInterpolator:
    ...
    def load_heightmap(self, filepath):
        """
        Load 128x128 PNG heightmap, automatically handle 8-bit and 16-bit PNG
        """
        try:
            # Load PNG with PIL
            ...
            if heightmap.shape != (128, 128):
                print(f"Warning: {filepath} size is {heightmap.shape}, resizing to 128x128")
                heightmap = cv2.resize(heightmap, (128, 128), interpolation=cv2.INTER_NEAREST)
