...
from .attention import BasicTransformerBlock, BasicAttetnionLayer, BasicCrosstransformerBlock
from models.denoising_diffusion_pytorch.swin_transformer import swin_s
...
class AttentionBlock_self_crosss(nn.Module):
    ...
    def forward(self, x, cond2):
        ...
        if self.use_checkpoint:
            return checkpoint(self._forward, (x, cond2), self.parameters(), self.use_checkpoint)
        else:
...
class AttentionBlock_trans(nn.Module):
    ...
    cross_only=False, #是否只使用交叉注意力
        ...
        self.proj_in = nn.Conv2d(channels,
                                ...
                                stride=1,
                                padding=0)
        self.cond_proj_in = nn.Conv2d(channels//4,
                                    inner_dim,
                                    kernel_size=1,
                                    stride=1,
                                    padding=0)
        ...
        if cross_only:
            self.transformer_blocks = nn.ModuleList(
                [BasicCrosstransformerBlock(inner_dim, num_heads, num_head_channels, dropout=dropout, context_dim=inner_dim)
                    for d in range(depth)]
            )
        else:
            self.transformer_blocks = nn.ModuleList(
                [BasicTransformerBlock(inner_dim, num_heads, num_head_channels, dropout=dropout, context_dim=inner_dim)
                    for d in range(depth)]
            )
        ...
        # )
        self.context_compressor = nn.Identity()
    ...
    def forward(self, x, context):
        ...
        context_dims = [64,128]
        ct_index = context_dims.index(self.in_channels//4) #根据输入的通道数，获取对应的上下文维度索引
        context = context[ct_index] #获取对应的上下文
        if context.dim() > 3:
            #插值并缩小到和x一样的尺寸
            context = self.context_compressor(context)  # (b, inner_dim, 32, 32)
            context = self.cond_proj_in(context)  # (b, inner_dim, 32, 32)
...
class RelationNet(nn.Module):
    def __init__(self, in_channel1=128, in_channel2=128, nhead=8, layers=3, embed_dim=128, ffn_dim=512,
        ...
        self.in_chennel1 = in_channel1

    def forward(self, cond, feat):
        # cluster = cluster.unsqueeze(0).repeat(feature.shape[0], 1, 1, 1)

        context_dims = [96, 192, 384, 768] #不同层级的上下文维度
        ct_index = context_dims.index(self.in_chennel1) #根据输入的通道数，获取对应的上下文维度索引
        cond = cond[ct_index]
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.init_cond_net = swin_s(in_channels=1)
                ...
                if ds in attn_res:        #如果当前下采样率在attn_res中，说明需要注意力机制
                    layers.append(        # 建立一个注意力块，输入通道数为ch，输出通道数为ch
                        AttentionBlock_trans(
                            ...
                            cross_only = True if ds in [1, 2] else False, #如果下采样率在1和2之间，使用自注意力，否则使用交叉注意力
                        )
                    )
        ...
        for level, mult in list(enumerate(channel_mults))[::-1]: #level【3,2,1,0】，mult【8,4,2,1】
            for i in range(res_blocks + 1):    
                ...
                # 结构图 cond2 特征通道投影模块（用于每层注入）
        self.cond2_projs_down = nn.ModuleList()
        self.cond2_projs_up = nn.ModuleList()

        self.cond2_projs_down.append(
            nn.Conv2d(1, int(64), kernel_size=1)
        )
        self.cond2_projs_down.append(
            nn.Conv2d(1, int(64), kernel_size=1)
        )

        for mult in channel_mults[:-2]:  # 下采样层数
            for _ in range(res_blocks):
                self.cond2_projs_down.append(
                    nn.Conv2d(1, int(mult * inner_channel), kernel_size=1)
                )

        self.cond2_projs_up.append(
            nn.Conv2d(1, int(512), kernel_size=1)
        )
        for mult in channel_mults[::-1]:  # 上采样层数（倒序，跳过最后一个）
    ...
    def forward(self, x, gammas,cond2):
        """
        ...
        :return: an [N x C x ...] Tensor of outputs.
        """
        cond2_copy = cond2.clone().type(torch.float32) #将输入的条件图像进行处理，得到一个新的条件图像

        cond2 = self.init_cond_net(cond2.type(torch.float32)) #将输入的条件图像进行处理，得到一个新的条件图像
        ...
        #     if isinstance(module[0], ResBlock) and getattr(module[0], 'down', False):   
        #         cond2_resized = F.interpolate(cond2, size=h.shape[-2:], mode='nearest')
        #         h = h + self.cond2_projs_down[down_idx](cond2_resized)
        #         down_idx += 1
        ...
        #     if isinstance(module[0], ResBlock) and getattr(module[0], 'down', False):
        #         cond2_resized = F.interpolate(cond2, size=h.shape[-2:], mode='nearest')
        #         h = h + self.cond2_projs_up[up_idx](cond2_resized)
        #         up_idx += 1
        ...
        # Initialize index counters for projection modules
        down_idx = 0
        
        # Process input blocks (downsampling path)
        for module in self.input_blocks:
            # Check if the module is a ResBlock and not a downsampling block
            # AND check if it doesn't contain an AttentionBlock_trans (which already has cross attention)
            contains_attention = False
            for submodule in module:
                if isinstance(submodule, AttentionBlock_trans):
                    contains_attention = True
                    break
            
            # Only add residual connections for blocks that don't do downsampling and don't have attention
            if isinstance(module[0], ResBlock) and not getattr(module[0], 'down', False) and not contains_attention and down_idx < len(self.cond2_projs_down):
                # Resize conditional feature map to match current feature map size
                cond2_resized = F.interpolate(cond2_copy, size=h.shape[-2:], mode='nearest')
                # Add projected conditional features as residual connection
                h = h + self.cond2_projs_down[down_idx](cond2_resized)
                down_idx += 1
        ...
        # Initialize index counter for upsampling path
        up_idx = 0
        
        # Process output blocks (upsampling path)
        for module in self.output_blocks:
            # Check if the module contains an AttentionBlock (which already has attention mechanism)
            contains_attention = False
            # Only add residual connections for blocks that don't do upsampling and don't have attention
            if isinstance(module[0], ResBlock) and not getattr(module[0], 'up', False) and not contains_attention:
                # Resize conditional feature map to match current feature map size
                cond2_resized = F.interpolate(cond2_copy, size=h.shape[-2:], mode='nearest')
                # Add projected conditional features as residual connection
            ...
            # Concatenate with skip connection and process the block
            h = torch.cat([h, hs.pop()], dim=1)
            h = module(h, emb, cond2)
        
        # Convert to original dtype and apply final convolution
        h = h.type(x.dtype)
        return self.out(h)
