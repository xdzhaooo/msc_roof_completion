...
class Palette(BaseModel):
    def __init__(
        ...
        **kwargs
        ...
        # set output layer to normal distribution
        #设置输出层为正态分布，均值为0，标准差为0.02
        ...
        # ema_conv_out = self.netG_EMA.denoise_fn.out[2]  # 或者 model.unet.out[2]，取决于你的结构
        # nn.init.normal_(ema_conv_out.weight, mean=0.0, std=0.02)
        # nn.init.zeros_(ema_conv_out.bias)
    ...
    def get_current_visuals(self, phase='train'):
        ...
        if phase == 'train':
            dict.update({
                'y0_hat': (self.y0hat.detach()[:].float().cpu()+1)/2,
            })
        dict.update({
            'gt_image': (self.gt_image.detach()[:].float().cpu()+1)/2,
            'cond_image': (self.cond_image.detach()[:].float().cpu()+1)/2,
            'control_image': (self.control_image.detach()[:].float().cpu()),
    ...
    def save_current_results(self):
        ...
        for idx in range(self.batch_size):
            ...
            control_img = torch.clamp(self.control_image[idx], -1, 1)
            ...
            y0_hat_path = os.path.join(self.opt['path']['results'],"y0hat_progress", str(self.epoch))
            os.makedirs(y0_hat_path, exist_ok=True)
            save_image((self.ret_y0[idx:-1:16]+1)/2, os.path.join(y0_hat_path, self.path[idx]))
    ...
    def train_step(self):
        ...
        for train_data in tqdm.tqdm(self.phase_loader): #train_data是一个字典，包含了训练数据，一个batch，tqdm是一个进度条库
            ...
            cond_mask = (rand_cond>p_random).float().view(-1,1,1,1)
            ...
            loss,self.y0hat = self.netG(self.gt_image, cond_image, mask=self.mask,control_image=self.control_image) #这里的mask就是footprint
            raw_loss = loss.clone()
            ...
            for val_data in tqdm.tqdm(self.val_loader):
                ...
                if self.opt['distributed']:
                    if self.task in ['inpainting','uncropping']:
                        self.output, self.visuals,self.ret_y0 = self.netG.module.restoration(cond_image, y_t=None, 
                            y_0=self.gt_image, mask=self.mask, sample_num=self.sample_num,control_image=self.control_image)
                    else:
                        self.output, self.visuals,self.ret_y0  = self.netG.module.restoration(cond_image, sample_num=self.sample_num)
                else:
                    if self.task in ['inpainting','uncropping']:
                        self.output, self.visuals,self.ret_y0  = self.netG.restoration(cond_image, y_t=None, 
                            y_0=self.gt_image, mask=self.mask, sample_num=self.sample_num,control_image=self.control_image)
                    else:
                        self.output, self.visuals,self.ret_y0  = self.netG.restoration(cond_image, sample_num=self.sample_num)
