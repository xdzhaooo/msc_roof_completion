...
class Network(BaseNetwork):
    def __init__(self, unet, beta_schedule, module_name='sr3',parameterization='eps', **kwargs):
        ...
        elif module_name == 'roof_modules':
            from .roof_modules.unet import UNet
        ...
        self.parameterization = 'v' #dummy set to v
    ...
    def set_new_noise_schedule(self, device=torch.device('cuda'), phase='train'):
        ...
        # Variational lower bound weights for v-prediction
        ...
        lvlb_weights[0] = lvlb_weights[1]  # Prevent zero weight at t=0
    ...
    def predict_start_from_v(self, y_t, t, v):
        # Predict x₀ from y_t and predicted v
        # x₀ = √ᾱ_t * y_t - √(1-ᾱ_t) * v
        return (
    ...
    def p_sample_ddim(self, y_t, t,t_prev, clip_denoised=True, y_cond=None, eta=0.0,control_image=None):  # 使用ddim进行逆向采样
        ...
        noise_level = extract(self.gammas, t, x_shape=(1, 1)).to(y_t.device)
        model_output = self.denoise_fn(torch.cat([y_cond, y_t], dim=1), noise_level, control_image)
    ...
    def restoration(self, y_cond, y_t=None, y_0=None, mask=None, sample_num=8,ddim_sample_steps=50,control_image=None): #恢复图像的完整过程
        ...
        y_t=self.q_sample(y_0=y_cond*y_cond_mask, sample_gammas=gamma_t1.view(-1, 1, 1, 1), noise=torch.randn(y_cond.shape).to(y_cond.device)) #加噪后的图像
    ...
    def forward(self, y_0, y_cond=None, mask=None, noise=None,control_image=None): #训练过程
        ...
        model_output = self.denoise_fn(torch.cat([y_cond, y_noisy*mask+(1.-mask)*y_0], dim=1), sample_gammas,control_image)
        if self.parameterization == "v":
        # Target is v = √ᾱ_t * ε - √(1-ᾱ_t) * x₀
        ...
        # Target is noise ε
            ...
            y0_hat = self.predict_start_from_noise(y_noisy, t, model_output)
                ...
                # ==== SNR Weighting Loss ====
        # 计算 ᾱ_t 和 SNR
        alpha_bar = sample_gammas.view(-1)  # 假设 sample_gammas 是 ᾱₜ
        snr = alpha_bar / (1. - alpha_bar + 1e-8)

        # 可调参数 gamma（经验值 5.0 比较好）
        gamma = 5.0
        snr_weight = torch.minimum(snr, torch.tensor(gamma, device=snr.device)) / snr

        # reshape 用于广播匹配 model_output 维度
        snr_weight = snr_weight.view(-1, 1, 1, 1)


        if mask is not None:
            mask_allone = torch.ones_like(mask)
            y0_hat = y0_hat * mask + y_0 * (1. - mask) #y0_hat部分的mask
            # Apply mask to both target and model output
            if hasattr(self.loss_fn, '__name__') and self.loss_fn.__name__ in ['masked_mse_loss', 'masked_l1_loss', 'combined_loss_with_masked_ssim', 'masked_l1_loss_samplewise']:
                ...
                #loss_y0 = self.loss_fn(mask * y_0, mask * y0_hat, mask) #y0_hat部分的loss
                loss_y0 = ssim_loss(mask * y_0, mask * y0_hat)
                loss = (loss_v*snr_weight.view(-1)).mean()+0.2*loss_y0 #加权平均
            else:
