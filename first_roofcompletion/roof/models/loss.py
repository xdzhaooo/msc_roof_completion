...
def masked_l1_loss_samplewise(output, target, mask):
    """
    返回每个样本（batch）对应的 masked L1 loss，shape [B]
    """
    B = output.shape[0]
    loss = torch.abs(output - target) * mask
    loss_per_sample = loss.view(B, -1).sum(dim=1)
    denom = mask.view(B, -1).sum(dim=1) + 1e-8  # 避免除 0
