...
def fit_img_postfix(img_path): #作用是将图片路径的后缀改为png
    if not os.path.exists(img_path) and img_path.endswith(".jpg"):#如果不存在这个路径，并且后缀是jpg，那么就改成png
...
class EdgeDataset(data.Dataset):
    ...
    def build_list(self):
        ...
        samples = []
        for edge_name in os.listdir(labels_path):
            #image_directories = os.path.join(images_path, directory_name)
            #for file_name_ext in os.listdir(image_directories):
            # file_name = os.path.basename(edge_name)
            # image_path = fit_img_postfix(os.path.join(images_path, directory_name, file_name))
            # lb_path = fit_img_postfix(os.path.join(labels_path, directory_name, file_name))
            edge_path = fit_img_postfix(os.path.join(labels_path, edge_name))
            samples.append(edge_path)
        return samples

    def __getitem__(self, index):
        edge_path = self.data_list[index]
        # edge_path = self.edge_paths[index]
        # img_path = self.img_paths[index]
        #img_name = os.path.basename(img_path)
        ...
        img_name = os.path.basename(edge_path)
        raw_size = (edge.shape[1], edge.shape[2])
        ...
        if self.normalize_to_neg_one_to_one:   # transform to [-1, 1]
            ...
            #img = normalize_to_neg_one_to_one(img)
        #return {'image': edge, 'cond': img, 'raw_size': raw_size, 'img_name': img_name}
        return {'image': edge, 'raw_size': raw_size, 'img_name': img_name}
