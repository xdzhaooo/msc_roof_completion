...
class LatentDiffusion(DDPM):
    ...
    def p_losses(self, x_start, t, *args, **kwargs):
        if self.start_dist == 'normal': #从正态分布中采样 n 0
            noise = torch.randn_like(x_start)
        elif self.start_dist == 'uniform':
            noise = 2 * torch.rand_like(x_start) - 1.
        else:
        ...
        if self.cfg.get('use_disloss', False):
            with torch.no_grad():
                ...
                edge_rec = self.first_stage_model.decode(x_rec / self.scale_factor) #从潜在空间解码，x_rec 除以 scale_factor 调整尺度
            ...
            loss_ce = SpecifyGradient.apply(x_rec, loss_tmp.mean()) #输出的是纯1占位符，目的是为了构建计算图
            # 应用 SpecifyGradient： 
            # - x_rec: 潜在表示 \hat{z}_0
    ...
    def cross_entropy_loss_RCF(self, prediction, labelf, beta=1.1):
        ...
        mask_temp = (label > 0) & (label <= 0.3) #范围是 (0, 0.3]
