...
def define_dataloader(logger, opt):
    ...
    # 检查是否需要分布式训练，兼容无accelerator的情况
    use_distributed = opt['distributed']
    if 'accelerator' in opt and opt['accelerator'] is not None and hasattr(opt['accelerator'], 'num_processes'):
        use_distributed = use_distributed or opt['accelerator'].num_processes > 1
        num_processes = opt['accelerator'].num_processes
        local_process_index = opt['accelerator'].local_process_index
    else:
        num_processes = 1
        local_process_index = 0
    
    if use_distributed: #分布式训练，分布式采样器 
        data_sampler = DistributedSampler(phase_dataset, shuffle=dataloader_args.get('shuffle', False), num_replicas=num_processes, rank=local_process_index) #分布式采样器，shuffle=True表示每个epoch都打乱数据
    ...
    #if opt['accelerator'].is_main_process and val_dataset is not None:
    if val_dataset is not None: #验证集不使用分布式采样器，val_dataset是一个数据集对象，val_dataloader是一个数据加载器对象
