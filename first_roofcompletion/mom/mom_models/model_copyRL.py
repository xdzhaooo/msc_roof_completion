...
class Trainer(BaseModel):
    def __init__(
        ...
        **kwargs
        ...
        self.netG.set_autoencoder(autoencoder) #将autoencoder传入网络模型中，autoencoder是一个类，包含了编码器和解码器的网络结构和参数
        self.netG_EMA.set_autoencoder(autoencoder) #将autoencoder传入网络模型中，autoencoder是一个类，包含了编码器和解码器的网络结构和参数
    ...
    def get_current_visuals(self, phase='train'):
        ...
        if phase == 'train':
            dict.update({
                ...
                #'roofline_noisy': (self.roofline_noisy.detach()[:].float().cpu()),
    ...
    def train_step(self):
        ...
        for k,train_data in enumerate(tqdm.tqdm(self.phase_loader)):

            self.set_input(train_data)

            if i == 0 and k == 0:
                if isinstance(self.netG, nn.parallel.DistributedDataParallel):
                    self.netG.module.on_train_batch_start(self.gt_image) #latent diffusion的特有方法，在第一个batch开始调整均值和方差设置缩放系数
                    self.netG_EMA.module.on_train_batch_start(self.gt_image)
                else:
                    self.netG.on_train_batch_start(self.gt_image)
                    self.netG_EMA.on_train_batch_start(self.gt_image)
            i += 1
            ...
            with autocast(enabled=self.accelerator.mixed_precision != 'no' if self.accelerator else False):
                ...
                loss, loss_discretize, roofline_y0_hat = self.netG.forward_latent(self.gt_image, cond_image, mask=self.mask, roofline_image=roofline_img)
                raw_loss = loss.clone()
                raw_loss_discretize = loss_discretize.copy()
            ...
            #self.accelerator.clip_grad_norm_(filter(lambda p: p.requires_grad, self.netG.parameters()), max_norm=10.0)
            self.roofline_y0_hat = roofline_y0_hat

            total_norm=0.0
            for parm in self.netG.parameters():
                if parm.grad is not None:
                    total_norm = total_norm + parm.grad.data.norm(2).item() ** 2
            total_norm = total_norm ** (1. / 2)


            if (k + 1) % accumulation_steps == 0 or (k + 1) == len(self.phase_loader):
            ...
            if self.accelerator.is_main_process:
                if self.iter % self.opt['train']['log_iter'] == 0:
                    ...
                    self.writer.add_scalar('Total Norm', total_norm)
                    for key, value in raw_loss_discretize.items():
                        self.writer.add_scalar(key, value)
                        self.logger.info('{:5s}: {}\t'.format(str(key), value))
    ...
    def val_step(self):
        ...
        try:
            ...
            for i, val_data in enumerate(tqdm.tqdm(self.val_loader, desc=f"Validation [Rank {rank}]")):
                ...
                # 记录推理开始时间
                start_time = time.time()
                self.output, self.visuals, self.roofline_arr, self.roofline_y0_arr, self.roofline_output = self.netG.restoration_latent(
                ...
                if self.accelerator.is_main_process:
                    for met in self.metrics:
                        ...
                        value = met(self.roofline_img, self.roofline_output)
