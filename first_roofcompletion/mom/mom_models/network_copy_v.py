...
class Network(BaseNetwork):
    def __init__(self, unet, beta_schedule, module_name='sr3', parameterization='eps', default_scale=False,**kwargs):
        ...
        self.scale_factor = 1.#latent diffusion的scale_factor
        ...
        self.parameterization = parameterization #latent diffusion的parameterization
        self.parameterization = 'v'
        self.scake_factor_initialized = False #latent diffusion的scale_factor_initialized
        self.default_scale = default_scale #latent diffusion的default_scale
    ...
    def set_new_noise_schedule(self, device=torch.device('cuda'), phase='train',accelerator=None): #设置新的噪声调度表
        ...
        gammas = np.cumprod(alphas, axis=0) #gammas = alpha_c
        ...
        self.register_buffer('sqrt_gammas', to_torch(np.sqrt(gammas))) #sqrt(gamma) = sqrt alphca cumprod
        self.register_buffer('sqrt_one_minus_gammas', to_torch(np.sqrt(1. - gammas))) #sqrt(1-gamma) = sqrt(1-alpha cumprod)
        ...
        self.register_buffer('posterior_mean_coef2', to_torch((1. - gammas_prev) * np.sqrt(alphas) / (1. - gammas)))
            ...
            # 计算变分下界权重 lvlb_weights
        if self.parameterization == "eps":
            lvlb_weights = betas ** 2 / (2 * posterior_variance * alphas * (1 - gammas))
        elif self.parameterization == "x0":
            lvlb_weights = 0.5 * np.sqrt(gammas) / (2. * (1 - gammas))
        elif self.parameterization == "v":
            lvlb_weights = np.ones_like(betas)  # v-prediction 通常使用均匀权重
        else:
            raise NotImplementedError(f"Unknown parameterization: {self.parameterization}")
        lvlb_weights[0] = lvlb_weights[1]  # 防止 t=0 时权重为 0
    ...
    def get_v(self, x, noise, gamma):
        # return (
        #         extract(self.sqrt_gammas, t, x.shape) * noise -
        #         extract(self.sqrt_one_minus_gammas, t, x.shape) * x
        # )
        sqrt_gamma = torch.sqrt(gamma)
        sqrt_one_minus_gamma = torch.sqrt(1. - gamma)
        return sqrt_gamma * noise - sqrt_one_minus_gamma * x
    
    def predict_start_from_v(self, y_t, gamma, v):
        """
        从 v 和 y_t 预测 y_0
        参数：
            y_t: 加噪数据
        ...
        sqrt_gamma = torch.sqrt(gamma)
        sqrt_one_minus_gamma = torch.sqrt(1. - gamma)
        return sqrt_gamma * y_t - sqrt_one_minus_gamma * v
    ...
    def p_sample_ddim_rl(self, y_t, t,t_prev, clip_denoised=True, y_cond=None, eta=1.0,control_image=None):  # 使用ddim进行逆向采样
        ...
        model_out=self.RoofLinedenoise_fn(y_t, noise_level,torch.cat([y_cond,control_image], dim=1))
        ...
        # )
        if self.parameterization == "eps":
            y0_hat = self.predict_start_from_noise(y_t, t=t, noise=model_out)
        elif self.parameterization == "x0":
            y0_hat = model_out
        elif self.parameterization == "v":
            gamma = extract(self.gammas, t, x_shape=(1, 1, 1, 1)).to(y_t.device)
            y0_hat = self.predict_start_from_v(y_t, gamma, model_out)
    ...
    def on_train_batch_start(self, roofline_image): #
        ...
        if self.scale_by_std and (not self.scale_by_softsign):
            if not self.default_scale and self.scale_factor == 1.:  #如果没有使用默认的scale，并且scale_factor为1，说明是第一次训练
    ...
    def forward_latent(self, y_0, y_cond=None, mask=None, noise=None,roofline_image=None,roofline_noise=None): #训练过程sssssssssssssssssssssssssss
        ...
        if self.parameterization == "v":
            sample_gammas = extract(self.gammas, t, x_shape=(1, 1)) #提取t时刻的gamma，形状为b,-》b,1,1
        #这一步的目的是在两个连续时间步之间随机采样一个值，使得训练中使用的 gamma 参数有一定的连续性和随机性。
        sample_gammas = sample_gammas.view(b, -1)#将采样的gamma值reshape成b,1
        ...
        y_noisy = torch.full_like(y_noisy, fill_value=-1.0) #将y_noisy的值全部设置为-1.0
        ...
        if mask is not None:
            ...
            roofline_noise_hat = self.RoofLinedenoise_fn(y_roofline_noisy, sample_gammas, torch.cat([y_cond, y0], dim=1))
            ...
            if self.parameterization == "eps":
                target = noise_roofline
                roofline_y0_hat = self.predict_start_from_noise_with_gamma(y_roofline_noisy, sample_gammas, roofline_noise_hat)
            elif self.parameterization == "x0":
                target = roofline_encode
                roofline_y0_hat = roofline_noise_hat
            elif self.parameterization == "v":
                target = self.get_v(roofline_encode, noise_roofline, sample_gammas.view(-1, 1, 1, 1))
                roofline_y0_hat = self.predict_start_from_v(y_roofline_noisy, sample_gammas.view(-1, 1, 1, 1), roofline_noise_hat)
            ...
            loss_simple += self.get_loss(roofline_noise_hat, target, mean=False).mean([1, 2, 3]) # : (B, 1)
            ...
            loss_vlb += torch.abs(roofline_y0_hat - roofline_encode).mean([1, 2, 3]) * rec_weight # : (B, 1)
            ...
            loss_dict.update({f'{prefix}/loss_out': loss_tmp.mean()}) # 更新损失字典，包含 loss_simple 和 loss_vlb

            loss = loss_simple + loss_vlb
            loss_simple_ce = loss_simple + loss_ce.mean()
    ...
    def restoration_latent(self, y_cond, y_t=None, y_0=None, mask=None, sample_num=8,ddim_sample_steps=50,roofline_image=None): #恢复图像的完整过程
        ...
        注意：
            ...
            self.ddim_sample_steps 控制实际采样步数（比完整时间步数少）。
        """
        print("###USING SCALE FACTOR IN RESTORATION###", self.scale_factor)
        device = self.accelerator.device if self.accelerator is not None else y_cond.device
        b, *_ = y_cond.shape
        # 构建 DDIM 采样的时间步序列，从 self.num_timesteps-1 到 0，共采样 self.ddim_sample_steps 步
        ddim_steps = ddim_sample_steps
        ...
        y_t_roof = torch.full_like(y_cond, fill_value=-1.0, device=device) #将y_t_roofline初始化为-1.0
        ...
        for t_val,t_prev in zip(timesteps, timesteps_prev):
            ...
            y_t_roofline,y_0_hat_roofline = self.p_sample_ddim_rl(y_t_roofline, t_tensor,t_tensor_prev, y_cond=y_cond,eta=0.0,control_image=y_0) #roofline的采样
            #y_t_roof,_ = self.p_sample_ddim_r(y_t_roof, t_tensor,t_tensor_prev, y_cond=y_cond,control_image=y_0_hat_roofline)
            # 如果是补全任务，保留已知区域
            if mask is not None and y_0 is not None:
                y_t_roof = y_0 * (1. - mask) + mask * y_t_roof
            ...
            ret_arr_roofline_y0.append(y_0_hat_roofline/self.scale_factor)
