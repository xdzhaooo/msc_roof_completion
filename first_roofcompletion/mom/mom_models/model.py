...
class Palette(BaseModel):
    def __init__(
        ...
        **kwargs
        ...
        print(f"Debug: accelerator = {accelerator}")
        super(Palette, self).__init__(accelerator=accelerator, **kwargs)
        print(f"Debug: self.accelerator = {getattr(self, 'accelerator', 'NOT_SET')}")
    ...
    def val_step(self):
        ...
        is_main_process = self.accelerator.is_main_process if self.accelerator else True
        
        print(f"Rank {rank}: Num processes {num_processes}")
        import time
        import traceback
        self.netG.eval()
