...
# from .attention import BasicTransformerBlock, BasicAttetnionLayer
    ...
    def forward(self, x, emb,cond2=None):
        for layer in self:
            if isinstance(layer, EmbedBlock):  # 如果是EmbedBlock，将emb传入
                x = layer(x, emb)
            elif layer.__class__.__name__ == "AttentionBlock_trans":
                #print("shape of cond2:",cond2.shape)
                x = layer(x, cond2)
            elif layer.__class__.__name__ == "RelationNet":
                #print("shape of cond2:",cond2.shape)
                x = layer(cond2, x)
            else:
                x = layer(x)
...
class AttentionBlock_trans(nn.Module):
    ...
    depth=1,
        ...
        self.proj_in = nn.Sequential(
            nn.Conv2d(channels, inner_dim, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(inner_dim, momentum=0.03, eps=1e-3),  # 使用BatchNorm2d替代GroupNorm  
        )

        self.cond_proj_in = nn.Sequential(
            nn.Conv2d(cond2_channels, cond2_channels, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(inner_dim, momentum=0.03, eps=1e-3),  # 使用BatchNorm2d替代GroupNorm  
        )
    ...
    def forward(self, x, context):
        ...
        ct_index = context_dims.index(self.cond2_channels)
        context = context[ct_index]
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        cond_cross_dim=None, #交叉注意力的条件维度
    ):
        super().__init__()
        ...
        self.init_input = nn.Sequential(
            nn.Conv2d(in_channel, ch, kernel_size=3, stride=1, padding=1), #输入层的卷积层，输入通道数为in_channel，输出通道数为ch，卷积核大小为3，步幅为1，填充大小为1
            nn.GroupNorm(num_groups=min(ch // 4, 8), num_channels=ch) #归一化层，通道数为ch，分组数为ch//4和8的最小值
        )

        self.init_activation = nn.SiLU() #激活函数，使用SiLU激活函数
            ...
            for _ in range(res_blocks):  #每个层级的残差块数量，默认为2
                ...
                if ds in attn_res:        #如果当前下采样率在attn_res中，说明需要注意力机制
                    layers.append(        # 建立一个注意力块，输入通道数为ch，输出通道数为ch
                        AttentionBlock(   #AttentionBlock_trans是一个注意力模块，输入通道数为ch，输出通道数为ch
                            ch,
                            use_checkpoint=use_checkpoint,
                            num_heads=num_heads,
                            num_head_channels=num_head_channels,
                            use_new_attention_order=use_new_attention_order,
                            #cond2_channels=f_condunet*mult, #cond_cross_dim
                        )
                    )
        ...
        self.output_blocks_struc = nn.ModuleList([])             #输出部分，上菜样部分
        self.output_blocks_height = nn.ModuleList([])            #输出部分，保持高度不变的部分
        for level, mult in list(enumerate(channel_mults))[::-1]: #level【3,2,1,0】，mult【8,4,2,1】
            for i in range(res_blocks + 1):    
                ...
                if level and i == res_blocks:
                    ...
                    ds //= 2
                self.output_blocks_struc.append(EmbedSequential(*layers_struc))
                self.output_blocks_height.append(EmbedSequential(*layers_height))
                self._feature_size += ch

        # 调制层：用于将y_roofnoisy和y_roofline与最终输出进行调制
        # 为struct分支生成scale和shift参数（针对输出通道数）
        self.modulation_struct = nn.Sequential(
            nn.Conv2d(1, out_channel * 2, 1),  # 输出2*out_channel维度，前out_channel维为scale，后out_channel维为shift
            nn.GroupNorm(num_groups=min(out_channel // 4, 8), num_channels=out_channel * 2),
            SiLU(),
        )
        
        # 为height分支生成scale和shift参数（针对输出通道数）
        self.modulation_height = nn.Sequential(
            nn.Conv2d(1, out_channel * 2, 1),  # 输出2*out_channel维度，前out_channel维为scale，后out_channel维为shift
            nn.GroupNorm(num_groups=min(out_channel // 4, 8), num_channels=out_channel * 2),
            SiLU(),
        )

        self.out_struct = nn.Sequential(
            normalization(ch),
            SiLU(),
            zero_module(nn.Conv2d(input_ch, out_channel, 3, padding=2 ** (dilations[-1] - 1), dilation=dilations[-1])),
        )

        self.out_height = nn.Sequential(
            normalization(ch),
            SiLU(),
            zero_module(nn.Conv2d(input_ch, out_channel, 3, padding=2 ** (dilations[-1] - 1), dilation=dilations[-1])),
        )
    ...
    def forward(self, x, gammas):
        ...
        :param x: an [N x 3 x ...] Tensor of inputs (B&W)
        :param gammas: a 1-D batch of gammas.
        :return: an [N x C x ...] Tensor of outputs.
        """
        y_noisy,y_roofnoisy,y_roofline=x[:,0,:,:],x[:,1,:,:],x[:,2,:,:]
        hs = []
        ...
        for i,module in enumerate(self.output_blocks_struc):
            h = torch.cat([h, hs.pop()], dim=1)
            h = module(h, emb)
        h = h.type(x.dtype)

        for i,module in enumerate(self.output_blocks_height):
            h_copy = torch.cat([h_copy, hs_copy.pop()], dim=1)
            h_copy = module(h_copy, emb)
        h_copy = h_copy.type(x.dtype)

        # 先通过输出层
        h = self.out_struct(h)
        h_copy = self.out_height(h_copy)

        # 应用调制层：将y_roofnoisy和y_roofline与最终输出进行调制
        # 确保y_roofnoisy和y_roofline的空间尺寸与输出匹配
        y_roofnoisy_resized = F.interpolate(y_roofnoisy.unsqueeze(1), size=h.shape[2:], mode='bilinear', align_corners=False)
        y_roofline_resized = F.interpolate(y_roofline.unsqueeze(1), size=h_copy.shape[2:], mode='bilinear', align_corners=False)
        
        # 生成调制参数 (scale, shift)
        modulation_struct_params = self.modulation_struct(y_roofnoisy_resized)
        modulation_height_params = self.modulation_height(y_roofline_resized)
        
        # 分离scale和shift参数
        struct_scale, struct_shift = torch.chunk(modulation_struct_params, 2, dim=1)
        height_scale, height_shift = torch.chunk(modulation_height_params, 2, dim=1)
        
        # FiLM调制：output = output * (1 + scale) + shift
        h = h * (1 + struct_scale) + struct_shift
        h_copy = h_copy * (1 + height_scale) + height_shift
