...
from mom_models import create_model, define_network, define_loss, define_metric
...
def main_worker(gpu, ngpus_per_node, opt):
    ...
    model = create_model(
        ...
        logger = phase_logger,
        writer = phase_writer,
        accelerator = None  # We do not use accelerator, so set to None
    )

    # Load pretrained models
    if 'pretrained_model_paths' in opt:
        if hasattr(model.netG, 'load_pretrained_models'):
            phase_logger.info('Loading pretrained models...')
            model.netG.load_pretrained_models(
                opt['pretrained_model_paths']['roof_path'],
                opt['pretrained_model_paths']['roofline_path']
            )
            phase_logger.info('Pretrained models loaded successfully')
