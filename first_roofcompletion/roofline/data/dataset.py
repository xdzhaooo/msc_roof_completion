...
class RoofDataset(data.Dataset): # data.Dataset is a class for loading,
    def __init__(
        ...
        self.control_imgs = control_imgs[:n_data]
    ...
    def _process_item(self, index: int):
        ...
        img_raw = torch.zeros_like(footprint) if self.no_height else self.loader(img_path) #0,255
        img = self.tfs(img_raw) 
        img = img*2 - 1 #归一化到[-1,1]之间
        #print("range of img: ", img.min(), img.max()) #范围是【0，1】之间

        # from torchvision.utils import save_image
        # save_image((torch.tensor(img)), f'./dgt_img.png')
    

        corrupted_path = self.corrupted_imgs[raw_idx] #输入缺损图像
        corrupted_img = self.resize(self.loader(corrupted_path))
        corrupted_img = self.height_normalize(corrupted_img) #归一化到[-1,1]之间
        ...
        control_path = self.control_imgs[raw_idx]  #带噪的高度图
        control_img = self.resize(self.loader(control_path))
        control_img = self.height_normalize2(control_img) #归一化到[-1,1]之间
        ...
        # from torchvision.utils import save_image
        # save_image((torch.tensor(img) + 1) / 2, f'./dgt_img.png')

        return img, img_path, mask, footprint, control_img,corrupted_img
    ...
    def __getitem__(self, index: int):
        ...
        gt_img, img_path, mask, footprint,control_img,corrupted_img = self._process_item(index)
        ...
        control_img=torch.from_numpy(control_img)
        control_img=control_img*footprint-(1-footprint)
