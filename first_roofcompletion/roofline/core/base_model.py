...
class BaseModel():
    def __init__(self, opt, phase_loader, val_loader, metrics, logger, writer):
        ...
        self.set_device = partial(Util.set_device, rank=opt['global_rank']) #partial函数，设置默认参数，因为本代码用单机多卡训练，所以rank参数是固定的local=global
    ...
    def load_network(self, network, network_label, strict=True):
        if self.opt['path']['resume_state'] is None: #如果没有预训练模型，则直接返回，在config.json中设置resume_state参数
            return 
        self.logger.info('Begin loading pretrained model [{:s}] ...'.format(network_label))

        print('Begin loading pretrained model [{:s}] ...'.format(network_label))

        model_path = "{}_{}.pth".format(self. opt['path']['resume_state'], network_label)
        #根据配置文件中的resume_state参数，加载预训练模型
        print(model_path)
        ...
        if isinstance(network, nn.DataParallel) or isinstance(network, nn.parallel.DistributedDataParallel):
            network = network.module #如果是多卡训练，则取module，因为多卡训练时，网络被封装在DataParallel中，在module下。
        #network.load_state_dict(torch.load(model_path, map_location = lambda storage, loc: Util.set_device(storage)), strict=strict)
        compatible_dict={}
        for key, param in torch.load(model_path, map_location = lambda storage, loc: Util.set_device(storage)).items():
            if key in network.state_dict() and network.state_dict()[key].shape == param.shape:
                compatible_dict[key] = param
            else:
                self.logger.warning('Pretrained network has unmatched key: {}'.format(key))
        network.load_state_dict(compatible_dict, strict=strict)
        #加载参数到网络中，map_location是一个函数，将参数加载到指定的设备上，并且设置严格模式，参数名必须一致
