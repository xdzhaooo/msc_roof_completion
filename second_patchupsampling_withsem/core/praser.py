...
# 打印 attr 这个类的 __init__ 方法接受的所有参数
def print_attr_params(attr):
    if hasattr(attr, '__init__'):
        sig = inspect.signature(attr.__init__)
        print("attr accepts parameters:", list(sig.parameters.keys()))
...
def init_obj(opt, logger, *args, default_file_name='default file', given_module=None, init_type='Network', **modify_kwargs):
    ...
    try:
        ...
        if isinstance(attr, type):  #判断attr是否是一个类
            # print_attr_params(attr) #打印attr这个类的__init__方法接受的所有参数
            # print("attr namd is:", attr)
            ret = attr(*args, **kwargs) #实例化类，*args表示任意多个参数，**kwargs表示关键字参数
            ret.__name__  = ret.__class__.__name__ #设置实例的名字

        elif isinstance(attr, FunctionType):  #如果attr是一个函数
...
def parse(args):
    ...
    if args.debug:
        ...
        opt['name'] = 'finetune_{}'.format(opt['name']) # 如果finetune_norm为True，则在name前加上finetune_
    else:
