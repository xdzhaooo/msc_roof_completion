...
class RoofPatchDataset(data.Dataset): # data.Dataset is a class for loading,
    def __init__(
        ...
        for img_path in self.imgs:
            ...
            padded_img = F.pad(img, padding=(self.patch_size[0] // 2, self.patch_size[1] // 2))
            patches_per_img = int(((padded_img.size(1) - 2*self.patch_size[0])/self.patch_size[0] + 1) * ((padded_img.size(2) - 2*self.patch_size[1])/self.patch_size[1] + 1))
    ...
    def _process_item(self, index: int): #调用__getitem__方法时，会调用这个方法
        ...
        # save_image((img_pad + 1) / 2, f'./debug/gt_pad_{index}.png')
        # save_image((torch.tensor(corrupted_img_pad) + 1) / 2, f'./debug/corrupted_pad_{index}.png')
        # save_image((mask_pad + 1) / 2, f'./debug/mask_pad_{index}.png')
        # save_image((footprint_pad + 1) / 2, f'./debug/footprint_pad_{index}.png')
        ...
        patches_per_row = int((img_pad.size(1) - 2*self.patch_size[0])/self.patch_size[0] + 1)
        patches_per_col = int((img_pad.size(2) - 2*self.patch_size[1])/self.patch_size[1] + 1)
        ...
        #crop
        img_patch = img_pad[:, patch_pos[0] * self.patch_size[0]: (patch_pos[0] + 2) * self.patch_size[0], patch_pos[1] * self.patch_size[1]: (patch_pos[1] + 2) * self.patch_size[1]]
        corrupted_img_patch = corrupted_img_pad[:, patch_pos[0] * self.patch_size[0]: (patch_pos[0] + 2) * self.patch_size[0], patch_pos[1] * self.patch_size[1]: (patch_pos[1] + 2) * self.patch_size[1]]
        mask_patch = mask_pad[:, patch_pos[0] * self.patch_size[0]: (patch_pos[0] + 2) * self.patch_size[0], patch_pos[1] * self.patch_size[1]: (patch_pos[1] + 2) * self.patch_size[1]]
        footprint_patch = footprint_pad[:, patch_pos[0] * self.patch_size[0]: (patch_pos[0] + 2) * self.patch_size[0], patch_pos[1] * self.patch_size[1]: (patch_pos[1] + 2) * self.patch_size[1]]

        # save_image((img_patch + 1) / 2, f'./debug/gt_patch_{index}_{patch_pos}.png')
        # save_image((torch.tensor(corrupted_img_patch) + 1) / 2, f'./debug/corrupted_patch_{index}_{patch_pos}.png')
        # save_image((mask_patch + 1) / 2, f'./debug/mask_patch_{index}_{patch_pos}.png')
        # save_image((footprint_patch + 1) / 2, f'./debug/footprint_patch_{index}_{patch_pos}.png')

        #loss mask
        loss_mask = torch.zeros_like(img_pad)
        loss_mask[:,pad_x: -pad_x, pad_y: -pad_y] = 1
        loss_mask_patch = loss_mask[:,patch_pos[0] * self.patch_size[0]: (patch_pos[0] + 2) * self.patch_size[0], patch_pos[1] * self.patch_size[1]: (patch_pos[1] + 2) * self.patch_size[1]]
        #save_image((loss_mask_patch), f'./debug/loss_mask_patch_{index}_{patch_pos}.png')
        ...
        grid_x, grid_y = np.meshgrid([patch_pos[0], patch_pos[0] + 1], [patch_pos[1], patch_pos[1] + 1], indexing='ij')
        #用meshgrid生成网格，grid_x是x坐标，grid_y是y坐标，形状为[2, 2]
        center_x = (grid_x+0.5)*self.patch_size[0]/img_pad.size(1) #将grid_x的值转换为中心点的坐标
        center_y = (grid_y+0.5)*self.patch_size[1]/img_pad.size(2) #将grid_y的值转换为中心点的坐标

        centers_xy = np.stack([center_x, center_y], axis=-1) #将center_x和center_y拼接起来，形状为[2, 2, 2]
        #相对于p



        return img, img_path, mask, footprint, corrupted_img, img_pad, corrupted_img_pad, mask_pad, footprint_pad, img_patch, corrupted_img_patch, mask_patch, footprint_patch,loss_mask_patch, centers_xy
    ...
    def __getitem__(self, index: int): #调用方法：dataloader[index]，会调用这个方法
        ...
        gt_img, img_path, mask, footprint, corrupted_img, gt_img_pad, corrupted_img_pad, mask_pad, footprint_pad, gt_img_patch, corrupted_img_patch, mask_patch, footprint_patch,loss_mask_patch, patch_pos = self._process_item(index)
        ori_img = gt_img
        gt_img = gt_img_patch
        ...
        ret['ori_img_size'] = [gt_img.size()[1], gt_img.size()[2]] #2*2的patch的大小
