...
class RoofPatchDataset(data.Dataset): # data.Dataset is a class for loading,
    def __init__(
        ...
        if lmdb_root is not None:
            self.lmdb_root = lmdb.open(lmdb_root, readonly=True, lock=False, readahead=False, meminit=False)
            with self.lmdb_root.begin() as txn:
                num_samples = txn.stat()["entries"]  # 获取数据库条目数量
                cursor=txn.cursor()
                index_list = [key.decode("ascii") for key, _ in cursor]#lmdb中的index组成的列表，有多少列数据
                n_data = data_len if data_len > 0 else len(index_list)
                self.index_list = index_list[:n_data]
            self.lmdb_root.close()
    ...
    def _process_item(self, index: int): #调用__getitem__方法时，会调用这个方法
        ...
        if self.lmdb_root is not None:
            self._init_db()
            key = self.index_list[raw_idx]
            with self.env.begin() as txn:
                img = txn.get(key.encode("ascii"))
                image_array = pickle.loads(img)
        ...
        if self.masks and len(self.masks) > 0:
        ...
        return img, img_path, mask, footprint, corrupted_img, img_pad, corrupted_img_pad, mask_pad, footprint_pad, loss_mask,first_output_smallsize
    ...
    def __getitem__(self, index: int): #调用方法：dataloader[index]，会调用这个方法
        ...
        gt_img, img_path, mask, footprint, corrupted_img, gt_img_pad, corrupted_img_pad, mask_pad, footprint_pad, loss_mask,first_output_smallsize = self._process_item(index)
        ori_img = gt_img
        gt_img = gt_img
    ...
    def __len__(self):
        ...
        if hasattr(self, 'index_list') and self.index_list is not None:
            return len(self.index_list) * len(self.down_res) * len(self.local_remove) * self.n_aug_deg * self.repeat
        else:
            return len(self.imgs) * len(self.down_res) * len(self.local_remove) * self.n_aug_deg * self.repeat
