...
class RoofDataset(data.Dataset): # data.Dataset is a class for loading,
    ...
    def _process_item(self, index: int): #调用__getitem__方法时，会调用这个方法
        ...
        return img, img_path, mask, footprint, corrupted_img
    ...
    def __getitem__(self, index: int): #调用方法：dataloader[index]，会调用这个方法
        ...
        #     from torchvision.utils import save_image
        #     #Image.fromarray(((cond_img.numpy()[0] + 1) / 2 * 10 * 256).astype('uint16')).save('./debug/' + os.path.basename(ret['path'])) #保存图片
        #     save_image((torch.tensor(cond_img) + 1) / 2, f'./debug/building_aftermask{index}.png')
        #     save_image((torch.tensor(mask) + 1) / 2, f'./debug/mask{index}.png')
        ...
        if self.recover_real_height: #恢复真实高度
            ...
            cond_img /= 2
            cond_img *= self.height_normalize2.height_range
            cond_img += self.height_normalize2.mid_height
            cond_img *= 256
        ...
        ret['height_range_corrupted'] = self.height_normalize2.height_range
        ret['mid_height_corrupted'] = self.height_normalize2.mid_height
